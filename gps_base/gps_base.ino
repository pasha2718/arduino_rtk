...
#include "git_version.h"                    // generated by Makefile with just the GIT_VERSION define
...
void RunSurvey()
{
  ...
  // There are two parameters to survey, a minimum time in seconds
  // and minimum 3D standard deviation in meters
  // it only exits once BOTH are achieved
  // we have set required accuracy to 5 meters and it is usually less than 2
  response = MY_GPS.enableSurveyMode(surveyMinutes * 60, 3.000);
  ...
  while (MY_GPS.svin.valid == false) {
    ...
    if (response == true) {
      ...
      */
      ...
      // we used to show lat/long here but it just taxes the system and flirts with crash
      LCD.setCursor(0, 1);
      LcdPad("Sat", 20);    // clear the line first to avoid leftover digits
      LCD.setCursor(4, 1);
